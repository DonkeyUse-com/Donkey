...
- A stationary spawned cursor shows the submitted turn first, then compact progress or response text in its label. Labels render complete words and wrap to their measured size instead of revealing partial typewriter text or clipping at the panel edge. Conversational responses, clarifications, review waits, failures, and other attention states stay visible so the user can read them and follow up; completed action-only cursors may fade out. Spawn labels expand wider on hover when that reduces wrapping, and select the cursor on click. Clicking a tasked label turns the same agent-colored surface into an inline editor with the agent message above and a lighter color-matched user input area below; Return submits the typed follow-up to that spawn's task, Shift-Return inserts a newline, Escape returns to the label, and clicking outside the editor closes it.
...
- Agent visualization overlays are non-interactive, non-activating panels. They may point, explain, and replay what the agent is doing or would do, but they do not synthesize mouse movement. Live keyboard or Accessibility actions remain separate guarded runtime actions. When a local-app dry run is based on an observation, preserve target-window geometry and control bounds on the dry-run steps so cursor replay can use pixel-grounded element positions instead of invented coordinates.
...
- Normal local-app tasks can produce visualization steps for planning, observing, navigating, acting, and verifying. The visualization should target observed control centers from Accessibility or screenshot understanding when those bounds exist, and must not claim completion unless the runtime verification state supports it.
