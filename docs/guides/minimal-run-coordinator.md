...
Donkey also supports the first generic local-app task intent and dry-run adapter boundary. `TaskIntent` can represent a parsed task, target app, normalized entities, parser source, confidence, and confirmation state. `LocalAppTaskDefinition` describes app-task knowledge as data: target app metadata, deterministic trigger terms, required entities and aliases, workflow steps, and verification rules. `LocalAppTaskIntentParser` can parse common local commands without a model call when a task definition is available, and `LocalAppTaskAdapter` can build a target-app local-navigation metadata request, project dry-run workflow steps, emit guarded keyboard command templates for an injected action backend, and verify observed visible text against the requested entity. Weather lookup is only the first benchmark definition for this generic boundary; the implementation is not Weather-specific. Default app launch/focus, default OS typing, Accessibility actions, app-knowledge retrieval, and live result verification remain unsupported.
