...
user command: "show me the weather for SF"
  ...
  -> launch or focus app
  -> observe app state through Accessibility and bounded screenshots
...
- Common app commands must use local intent parsing before remote planning; deterministic parsing is validator/benchmark support, not the submitted-command fallback.
...
- current slice: generic local-app task catalog, local-model command parser, deterministic parser validator, app availability check, and dry-run adapter boundary, with Weather lookup represented as built-in benchmark data.
- `d6e48c9`: archived plans, which was premature for this milestone.

The code now includes the first generic local-app task catalog, local JSON/JSONL task-definition loader, local task context provider, Accessibility control discovery/action planning, document form-fill review planner, local-LLM sidecar command parser, deterministic parser retained as validator/benchmark code, app availability check, dry-run adapter boundary, local-app live runner, macOS launch/focus, keyboard-input and Accessibility-input backends, pointer-prompt typed and voice command wiring, sample app-task definitions across weather/media/document domains, YOLO26 screenshot segmentation model-candidate metadata and process-backed sidecar backend, local UI-understanding sidecar adapter wired into live observation over bounded screenshots, Parakeet-only local voice-transcription routing and sidecar runtime, shared JSON sidecar runner, first-run local runtime setup UI, package-time bundled sidecar runner packages, setup-time model-weight download/cache operation including Ollama `qwen3:8b` for the local command-parser LLM, manifest-backed runtime install/download/cache/registration, checksum/signature metadata enforcement, sidecar health recheck, semantic memory retrieval contracts, redaction hooks, aggregate model-call observability, provider-decoded memory proposal handling, expanded latency reporting, and a package script that produces `dist/Donkey.app`. It does not yet include fully bundled Python/native backend environments for Parakeet, YOLO, and UI understanding, real UI-understanding model internals, a self-contained non-Ollama LLM runner, or release-key cryptographic signature verification.

## What Should Be Done Next

1. Package or bootstrap the remaining local backend dependencies behind the one setup button: Parakeet needs its local NVIDIA NeMo/Python backend, YOLO needs an Ultralytics-compatible backend and model cache, UI understanding needs a real local model backend, and the command-parser LLM needs either an accepted Ollama prerequisite or a self-contained runner.
2. Finalize release-key cryptographic signature verification, add behind-the-scenes repair/remove flows, and keep setup retryable without exposing per-runtime customization.
3. Keep metric collection as background system behavior using the existing metadata fields for command parse, launch/focus, observation, Accessibility action, keyboard action, verification, YOLO segmentation, UI-understanding, and Parakeet transcription.
4. Keep the current dry-run/guarded-live safety boundary: local or online LLM output can only become validated intent, task definitions, app-knowledge updates, observation summaries, transcripts, memory proposals, or planner hints, never direct input.
